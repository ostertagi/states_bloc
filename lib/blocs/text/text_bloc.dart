...
class TextBloc extends Bloc<TextEvent, TextState> {
  ...
  void mapEventToState(TextEvent event, Emitter<TextState> emit) async {
    ...
    if (event is BoldTextEvent) {
      _bold = event.value;
      print("BoldTextEvent ${event.value.toString()}");
      _eventBus.fire(const MessageEvent("Abdelghani Text Bold"));
    }

    if (event is ItalicTextEvent) {
      _italic = event.value;
      _eventBus.fire(const MessageEvent("Abdelghani Text Italic"));
    }
    emit(getBlocState());
  }
